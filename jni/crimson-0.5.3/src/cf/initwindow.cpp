...
#else //ANDROID
  gtypewidget = new CycleWidget( IW_WIDGET_TYPE,
    xoff, 4, w - xoff - 5, wdh,
    0, typelbl, CFOptions.GetGameType(), typelabels, this );
  gtypewidget->SetHook( this );

  mtypewidget = new CycleWidget( IW_WIDGET_MAPTYPE,
    gtypewidget->LeftEdge(), gtypewidget->TopEdge() + wdh + 4, gtypewidget->Width(), wdh,
    0, mapslbl, 0, mapslabels, this );
  mtypewidget->SetHook( this );
  CFOptions.SetCampaign( false );

  diffwidget = new CycleWidget( IW_WIDGET_DIFFICULTY,
    mtypewidget->LeftEdge(), mtypewidget->TopEdge() + wdh + 4, mtypewidget->Width(), wdh,
    0, hcaplbl, 0, difflabels, this );

  wd = new DropWidget( IW_WIDGET_OPTIONS,
    diffwidget->LeftEdge(), diffwidget->TopEdge() + wdh + 4, diffwidget->Width(), wdh,
    0, MSG(MSG_B_OPTIONS), this );
  wd->SetHook( this );

  levwidget = new TextListWidget( IW_WIDGET_LEVELS,
      6, wd->TopEdge() + wdh + 4, w-11, (h - (wdh + 4) * 5)/2 - 4,
      &levels, -1, WIDGET_HSCROLLKEY|WIDGET_VSCROLLKEY|
      (CFOptions.GetGameType() == GTYPE_NET_CLIENT ? WIDGET_DISABLED : 0), NULL, this );
  levwidget->SetHook( this );

  maxmap = Rect( levwidget->LeftEdge(), levwidget->TopEdge() + levwidget->Height() + 4, levwidget->Width(), levwidget->Height() );
